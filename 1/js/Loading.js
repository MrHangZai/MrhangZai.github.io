...
var MF_Loading=(function(){
	...
	function MF_Loading(p){
		...
		this.start=function(){//加载开始
			...
			this.set=setInterval(__up, _s.timers);
		};
		
		this.add=function(__o){
			return typeof __o === 'object' ? addLoadObject(__o) : null;
		};
		...
		this.addImgSheet=function(){//添加图片动画帧 参数imgPrefix,imgType,start,length,step
            ...
            for(var i=arg.start, lgt = arg.length; i <= lgt; i+=arg.step){
                var pindex = i;
                pindex = mask.toString() + i;
